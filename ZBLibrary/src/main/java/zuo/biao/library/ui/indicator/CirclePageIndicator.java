package zuo.biao.library.ui.indicator;
...
public class CirclePageIndicator extends View implements ViewPager.OnPageChangeListener {
    ...
    public CirclePageIndicator(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = getResources().obtainAttributes(attrs, R.styleable.CirclePageIndicator);
        //获取自定义属性的个数
        int N = typedArray.getIndexCount();
        for (int i = 0; i < N; i++) {
            int attr = typedArray.getIndex(i);
            if (attr == R.styleable.CirclePageIndicator_indicator_icon) {//通过自定义属性拿到指示器
                mIndicator = typedArray.getDrawable(attr);

            } else if (attr == R.styleable.CirclePageIndicator_indicator_margin) {
            ...
            } else if (attr == R.styleable.CirclePageIndicator_indicator_smooth) {
