package zuo.biao.library.swichlayout;
...
public abstract class SwitchLayout {
    ...
    public static long animDuration = 1000;
    ...
    public static void RotateHorizontal(Activity context,
                                        boolean isCloseActivity, Interpolator interpolator) {
        objAnim = ObjectAnimator.ofFloat(getRootView(context), "rotationY",
                180, 0, 0);
        if (interpolator != null) {
            objAnim.setInterpolator(interpolator);
        }
        objAnim.setDuration(longAnimDuration);
        if (isCloseActivity) {
            objAnim.addListener(animatorListener);
        }
        objAnim.start();
